...
const Pool = () => {
    ...
    const { write: approveLP } = useContractWrite({
        ...
        args: [MAPNETTOADDRESS[CONTRACT_ADDRESSES.ROUTER], to_wei('1000000000')],
    ...
    async function returnAmount(args: string[]) {
        return await chain_query({
            ...
            f_args: [MAPNETTOADDRESS[CONTRACT_ADDRESSES.FACTORY], to_wei('1'), ...args],
        });
    }
    ...
    async function calcPooledUserAsset(pair: Pair) {
        try {
            ...
            let pooledA =
                (parseFloat(from_wei(pair?.balance)) / parseFloat(from_wei(pair?.totalSupply))) *
                parseFloat(from_wei(pair?.reserve0));
            let pooledB =
                (parseFloat(from_wei(pair?.balance)) / parseFloat(from_wei(pair?.totalSupply))) *
                parseFloat(from_wei(pair?.reserve1));
                            ...
                            {pairs.map((el, i) => (
                                <li onClick={() => handlePairClick(el)} key={i}>
                                    <span className="pair-logo">
                                        <span className="move">{createElement(selectList[0].tokenIcon)}</span>
                                        <span>{createElement(selectList[1].tokenIcon)}</span>
